...
def drawLog():
    ...
    fig, ax = plt.subplots(figsize=(17, 10))
    ax.bar(df['productname'], df['total_ordered'], label='Total Ordered')
    ax.set_xticklabels(df['productname'], rotation=45, ha='right')
    ax.set_xlabel('Product Name')
    ax.set_ylabel('Total Ordered')
    ax.set_title('Top 5 Product Name by Total Ordered')
    ax = df.plot(kind='bar', color=['red', 'blue'])
    ax.set_title('Total Orders vs Quantitity in stock')
    ax.set_xlabel(df['productname'], rotation=45, ha='right')
    ax.set_ylabel('Total Ordered')
    return(fig)
